...
- **`client/` is a static Vercel-ready DID crypto watchlist using public Binance Spot data.** It
  streams live 24-hour tickers without an API key, answers common market questions through a
  rule-based agent, and signs portable price snapshots locally; private seeds never leave tab memory.
